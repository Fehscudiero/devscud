...
import {
  Globe, Gauge, Settings, LifeBuoy, Search, ShoppingCart, ChevronLeft, ChevronRight
} from "lucide-react";
...
const Servicos = () => {
  ...
  return (
    <section id="servicos" className={`py-20 min-h-[800px] flex items-center relative overflow-hidden transition-colors duration-1000 ${sectionBg}`}>

      {/* Background Cinético */}
      <div className={`absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[800px] h-[800px] blur-[150px] opacity-20 pointer-events-none transition-colors duration-1000 ${isDarkTheme ? 'bg-indigo-600' : 'bg-emerald-200'}`} />
      ...
      <div className="container mx-auto px-6 relative z-10">
        ...
        <div className="mb-12 text-center lg:text-left">
          ...
          <h2 className={`text-4xl lg:text-6xl font-black tracking-tighter ${isDarkTheme ? 'text-white' : 'text-slate-900'}`}>
            SOLUÇÕES <span className={`bg-gradient-to-r ${accentGradient} bg-clip-text text-transparent italic`}>DIGITAIS_</span>
          </h2>
        </div>
        ...
        <div className="flex flex-col lg:flex-row gap-12 items-center">
          ...
          {/* Navegação Mobile/Desktop (Esquerda) */}
          <div className="hidden lg:flex flex-col gap-4 w-1/3">
            {services.map((s, i) => (
              <button
                key={i}
                onClick={() => setActiveIndex(i)}
                className={`text-left p-4 rounded-xl transition-all duration-300 border-l-4 ${activeIndex === i
                    ? `bg-white/5 border-cyan-500 translate-x-4`
                    : `border-transparent opacity-30 hover:opacity-100`
                  }`}
              >
                <span className={`block text-[10px] font-mono ${accentColor}`}>{s.code}</span>
                <span className={`text-lg font-bold ${isDarkTheme ? 'text-white' : 'text-slate-900'}`}>{s.title}</span>
              </button>
            ))}
          </div>

          {/* Card em Foco (Centro/Mobile) */}
            ...
            <AnimatePresence mode="wait">
              <motion.div
                ...
                className={`w-full max-w-[600px] p-8 lg:p-16 rounded-[3rem] border-2 shadow-2xl relative overflow-hidden
                  ${isDarkTheme ? 'bg-zinc-900/50 border-white/10' : 'bg-white border-slate-100'}`}
              >
                {/* HUD Elements Internos */}
                <div className="absolute top-8 right-8 text-[10px] font-mono opacity-30 uppercase tracking-widest">
                  Status: Active <span className="animate-pulse text-green-500">●</span>
                </div>

                <div className="flex flex-col items-center lg:items-start text-center lg:text-left gap-8">
                  <div className={`p-6 rounded-3xl bg-gradient-to-br ${isDarkTheme ? 'from-white/10 to-transparent' : 'from-slate-100 to-white shadow-inner'}`}>
                    {(() => {
                  ...
                  <div className="space-y-4">
                    <h3 className={`text-3xl lg:text-5xl font-black tracking-tight ${isDarkTheme ? 'text-white' : 'text-slate-900'}`}>
                      {services[activeIndex].title}
                    </h3>
                    <p className={`text-lg leading-relaxed ${isDarkTheme ? 'text-slate-400' : 'text-slate-600'}`}>
                      {services[activeIndex].description}
                    </p>
                  </div>

                  <div className={`h-1 w-24 rounded-full bg-gradient-to-r ${accentGradient}`} />
                </div>
              </motion.div>
            </AnimatePresence>

            {/* Controles de Navegação Imersivos */}
            <div className="flex gap-6 mt-12">
              <button onClick={prevService} className={`p-4 rounded-full border border-white/10 hover:bg-white/10 transition-all ${isDarkTheme ? 'text-white' : 'text-slate-900'}`}>
                <ChevronLeft size={32} />
              </button>
              ...
              <div className="flex items-center gap-2">
                {services.map((_, i) => (
                  <div key={i} className={`h-1.5 transition-all duration-500 rounded-full ${activeIndex === i ? `w-8 bg-cyan-500` : `w-2 bg-white/20`}`} />
                ))}
              </div>

              <button onClick={nextService} className={`p-4 rounded-full border border-white/10 hover:bg-white/10 transition-all ${isDarkTheme ? 'text-white' : 'text-slate-900'}`}>
