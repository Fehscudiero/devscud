...
const Testimonials = () => {
  ...
  // Efeito de Parallax ajustado para a direita
  const { scrollYProgress } = useScroll({
    target: sectionRef,
    offset: ["start end", "end start"],
  });

  // Alterado para mover da direita para a esquerda (ou vice-versa) no topo
  ...
  return (
    <section
      ref={sectionRef}
      id="testimonials"
      className={`py-32 relative transition-colors duration-1000 z-30 overflow-hidden ${sectionBg}`}
    >
      {/* MARCA D'ÁGUA POSICIONADA NA DIREITA CIMA */}
      ...
      <div className="container mx-auto px-6 relative z-10">
        ...
        <div className="grid md:grid-cols-3 gap-8">
          {testimonials.map((testimonial, index) => (
            ...
            >
              <div
                className={`absolute inset-0 rounded-[2.5rem] opacity-0 group-hover:opacity-10 transition-opacity duration-500 pointer-events-none bg-gradient-to-br ${accentGradient}`}
              />

              <div
                className={`absolute -top-4 left-10 px-5 py-1.5 rounded-full text-[10px] font-black uppercase tracking-widest shadow-xl 
                ${
                  isDarkTheme
              ...
              <Quote
                className={`absolute top-10 right-10 w-12 h-12 opacity-10 ${accentColor}`}
              />
              ...
              <div className="flex items-center gap-4 pt-8 border-t border-white/5">
                <div className="relative">
                  <div
                    className={`w-14 h-14 rounded-2xl flex items-center justify-center font-black text-xl shadow-inner transform group-hover:rotate-6 transition-transform duration-500
                  ...
                  <div className="absolute -bottom-2 -right-2 bg-white dark:bg-slate-900 rounded-full p-1 shadow-lg">
                    <BadgeCheck
                      className={`w-5 h-5 ${
                        isDarkTheme ? "text-cyan-400" : "text-emerald-500"
                      }`}
                    />
                  </div>
                </div>
                ...
                <div>
                  ...
                  <p
                    className={`text-xs font-mono uppercase tracking-wider ${accentColor}`}
                  >
                    {testimonial.role}
                  </p>
                </div>
      ...
      <div
        className={`absolute bottom-0 left-0 w-[500px] h-[500px] rounded-full blur-[150px] opacity-10 pointer-events-none 
